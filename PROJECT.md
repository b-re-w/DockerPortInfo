...
- **웹 서버**: primary 서버에서 **user 권한**으로 실행되는 FastAPI 앱.
  - 모든 라우트는 공통 prefix `BASE`(=`DOCKERPORTINFO_BASE_PATH`, 기본 `/info/docker`) 아래에 둔다.
    nginx `location /info/docker/` 하나로 프록시하기 위함.
  - 수신 API `POST BASE/{server_name}` — `docker ps` 원본 텍스트를 받는다.
  - 조회 API `GET BASE/{server_name}`, `GET BASE/snapshots` — 정리된 자료형을 JSON으로 반환.
  - 페이지 `GET BASE/` — 정적 HTML/CSS/JS (`BASE/static/*`). 서버가 HTML에 base를 주입해 자산/fetch가 prefix를 따른다.
...
- **프론트엔드**: 렌더 후 `setInterval`로 1분마다 `BASE/snapshots`를 폴링해 화면을 다시 그린다.

```
[primary]   crontab(1m) ─ send_docker_ps.sh ─ docker ps ─┐
                                                          ├─POST BASE/<name>─▶ FastAPI(primary:13000) ─ store(메모리+디스크)
[secondary] crontab(1m) ─ send_docker_ps.sh ─ docker ps ─┘   (nginx https → 13000)      │
                                                                                        ▼
                                                              브라우저 ─GET BASE/snapshots(1m 폴링)─ 화면 갱신
```
...
- `ServerSnapshot`: `server_name`, `updated_at`, `container_count`, `total_containers`, `containers[]`, `raw?`
...
- **접속 IP별 포트 비공개**: 조회 API/페이지는 인증이 없지만, `DOCKERPORTINFO_CAMPUS_CIDRS`(기본
  `168.188.0.0/16`) 밖의 클라이언트에게는 컨테이너 **포트 정보를 응답에서 제거**하고 안내 문구를 내려준다.
  클라이언트 IP는 nginx의 `X-Real-IP`/`X-Forwarded-For`로 판별. 이미지·서버 구성은 학외에도 공개.
  (차단이 아니라 부분 마스킹이라, 학외에서도 어떤 세션이 떠 있는지는 보이되 접속 포트는 숨긴다.)
